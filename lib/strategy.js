...
class Strategy {
  ...
  authenticate(req, options) {
    ...
    // Since the remember me cookie is primarily a convenience, the lack of one
    // is not a failure.  In this case, a response should be rendered indicating
    // a logged out state, rather than denying the request.
    if (!token) {
      return this.pass()
    }
    ...
    function verified(err, user, info) {
      ...
      if (!user) {
        ...
        //
        // Note that a failure at this point may indicate a possible theft of
        // the cookie.  If handling this situation is a requirement, it is up to
        // the application to encode the value in such a way that this can be
        // detected.
      ...
      const issued = (err, val) => {
        ...
        req.res.cookie(self._key, val, self._opts)
